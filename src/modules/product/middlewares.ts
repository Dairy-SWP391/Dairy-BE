          ...
          options: async (value, { req }) => {
            ...
            try {
              ...
              if (user.role !== ROLE.MEMBER) {
                throw new ErrorWithStatus({
                  message: PRODUCT_MESSAGES.USER_MUST_BE_MEMBER,
                  status: HTTP_STATUS.UNAUTHORIZED,
                });
              }
              req.decoded_authorization = decoded_authorization;
...
export const addProductToWishListValidator = validate(
  checkSchema({
    product_id: {
      notEmpty: { errorMessage: PRODUCT_MESSAGES.PRODUCT_ID_IS_REQUIRED },
      isNumeric: { errorMessage: PRODUCT_MESSAGES.PRODUCT_ID_MUST_BE_NUMBER },
      custom: {
        options: async (value, { req }) => {
          ...
          if (product) {
            throw new Error(PRODUCT_MESSAGES.PRODUCT_ALREADY_ADD_TO_WISHLIST);
          }
          return true;
        },
      },
